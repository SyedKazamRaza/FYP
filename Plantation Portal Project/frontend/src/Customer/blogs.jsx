      ...
      { !count && 
        <section className="breadcrumbs-custom-inset">
          <div className="breadcrumbs-custom context-dark bg-overlay-33">
            ...
            <div className="box-position"  style={{backgroundImage: `url(${process.env.PUBLIC_URL + '/images/blogs.jpg'})`}}></div>
          </div>
        </section>
      }
        ...
        <div className="container">
          ...
          {blogs.slice(0,count).map(blog => (
            <div className="col-md-6 col-lg-4 col-xl-4 wow fadeInRight" data-wow-delay="0s" key={blog._id}>
              <article className="team-classic"><Link className="team-classic-figure" to="#"><img src={blog.image} alt="" style={{width:"370px", height:"406px"}} /></Link>
                <div className="team-classic-caption">
                  <h5 className="team-classic-name"><Link to={`/blogs/${blog._id}`}>{ blog.title }</Link></h5>
                </div>
              </article>
            </div>
          ))} 
